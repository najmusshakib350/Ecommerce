...
//delete aal cartItem
module.exports.deleteAllCartItems = catchasync(async function (req, res, next) {
  let cart = await Cart.find({ user: req.user._id });

  cart.forEach(async (el, i) => {
    let count = el.product.quantity + el.count;
    await Product.findByIdAndUpdate(el.product._id, { quantity: count });
  });
  await Cart.deleteMany({ user: req.user._id });

  res.status(204).json({
    status: "cartItems delete successfully!!!",
  });
});
